...
static void avi_write_data(FILE *file, unsigned char *data, int size)
{
...
static int avi_skip_usr_data(FILE *file)
{
	int tag, size;
	avi_get_chunk(file, &tag, &size);

	if (tag!=GM_MAKE_FOURCC('J','U','N','K') && tag!=GM_MAKE_FOURCC('m','o','v','i'))
	{
...
HANDLE GMAVIOpen(char *filename, int mode ,int size)
{
    ...
    if(GMAVI_IS_CREATE(mode)&&!is_existed)
        strcpy_s(file_mode, 4,"wb+");
    else if(GMAVI_IS_READ(mode) || GMAVI_IS_WRITE(mode))
        strcpy_s(file_mode, 4, "rb+");
    else
    ...
    if(err == 0)
    {
        ...
        if(GMAVI_IS_CREATE(mode))
        {
            ...
            {
                strcpy_s(file_mode, 4, "wb+");
                sprintf_s(tmp_str, 512, "%s_idx", filename);
                err = fopen_s (&pIdxFile, tmp_str, file_mode);
                if(err == 0 && !pIdxFile)
                {
                    perror("Open1");
                    goto GMSTS_OPEN_FAILED;
  ...
  //		junk_size1 = avi_skip_usr_data(avi_file->file);
  //      junk_size1 += avi_skip_junk(avi_file->file);

        //movi
        ret = avi_get_list(avi_file->file, &movi_tag, &avi_file->movi_len);
		//printf("movi_tag:%d (int)movi:%d avi_file->movi_len:%d\n",movi_tag, GM_MAKE_FOURCC('m','o','v','i'), avi_file->movi_len);
...
int GMAVISeekToFrame(HANDLE handle, int frame, int *pOffset)
{
	...
	int ret = -1;
	AviIndex  index;
    int streamid = GM_MAKE_FOURCC('0','0','d','c');
    int offset = 0;
