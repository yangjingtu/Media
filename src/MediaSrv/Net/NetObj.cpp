...
{ 
	...
	do 
	{
		...
		case ADP::ADP_CMD_REQ_STREAM_OPEN:
			...
			Msg.Body.RspStreamOpen.rVal = m_pStream->Open(GetUrl(m_StrUrl), AV_STREAM_MODE);
			if ( MW_SUCC != Msg.Body.RspStreamOpen.rVal )
			{
				m_pStream->Close();
...
S32 CNetObj::Run()
{
	...
	{	
		...
		if (pWFds && FD_ISSET(m_Sock.Handle(), pWFds)) 
		{
				SndAvStream(5);
		}
	}
