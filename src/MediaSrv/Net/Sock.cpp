...
S32 CSock::GetHostByName(const char *Name, char *Ip)
{
	...
	if (ht) 
	{
		sprintf( Ip, "%d.%d.%d.%d", ht->h_addr_list[0][0] & 0x00ff, \
...
S32 CSock::SetServer(string Link)
{
	...
	S32 PortIdx = 0;
	S32 Port[5] = {0};
	sscanf(Link.c_str(), "%d:%d:%d:%d:%d", &Port[0], &Port[1], &Port[2],&Port[3],&Port[4]);
	SOCK_ADDR_IN AddrSrv;  
	do {
		AddrSrv.sin_family = AF_INET;  
		AddrSrv.sin_port   = htons((U16)Port[PortIdx]);  
		AddrSrv.sin_addr.S_un.S_addr = htonl(INADDR_ANY);
		Ret = ::bind(m_sHandle,(SOCK_ADDR*)&AddrSrv, sizeof(SOCK_ADDR));
		if (!Ret)  
		{  
			DEBUG_ERROR("---------bind port[%d]---------\r\n", Port[PortIdx]);
			break; 
		}
	} while(++PortIdx < 5);
	if (PortIdx >= 5)
